...
const CsvExcelUpload = () => {
  ...
  const uploadFileToAPI = useCallback(
    debounce(async (file) => {
      ...
      try {
        setUploading(true);
        const response = await axios.post(`${apiUrl}/v1/import`, formData, {
          headers: {
        ...
        if (
          ...
          error.response.data.message.includes(
            'Undefined array key "question_number"'
          )
        ) {
          toast.error("Excelのフォーマットが正しくありません");
        } else {
      ...
      <div>
        ...
        <div className="ml-[20px]">
          <h3 className="font-semibold">
            書き方のアドバイスを変更するための Excel ファイルをアップロードする
          </h3>
          ...
          <div className="h-[30px] py-[50px] px-[20px]">
            ...
            {uploading && (
              <div
                className="my-1 py-[10px] px-[20px] bg-[#e0e0e0] text-[#000000] rounded-[5px]"
                role="alert"
              >
